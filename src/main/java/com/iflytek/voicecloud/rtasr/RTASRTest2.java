...
public class RTASRTest2 {
     ...
     * appId
     */
    private static final String APPID = "5ebcab1c";

    /**
     * appId对应的secret_key
     */
    private static final String SECRET_KEY = "fae560c806f5342dae0f34238349bc3d";
    ...
    // 音频文件路径
    private static final String AUDIO_PATH = "E:\\excelTest\\test_1.pcm";
    ...
    private static void executeTask() throws URISyntaxException, InterruptedException {
...
//        sendM1(client);
    ...
    private static void sendM1(MyWebSocketClient client) {
        // 发送音频
    ...
    private static void sendM2(MyWebSocketClient client) {
        // 发送音频
        byte[] bytes = new byte[CHUNCKED_SIZE];
        ...
        //rws	每当进行写操作，同步的刷新到磁盘，刷新内容和元数据
        //rwd	每当进行写操作，同步的刷新到磁盘，刷新内容
        try (FileInputStream raf = new FileInputStream(AUDIO_PATH)) {
            int len = -1;
            long lastTs = 0;
            while ((len = raf.read(bytes)) != -1) {
                if (len < CHUNCKED_SIZE) {
